...
prior = priorHandle(0.51);

FI = 14.8 * prior(stmSpc);
estVar = 1 ./ FI;
...
noiseVar = estVar + 0.1 * mean(estVar);
noiseFI = 1 ./ noiseVar;
...
%% Search procedure
para_woFB = fmincon(@(x) lossFunc(x, 0.59, 12.41, 0.0), 0.5, [], [], [], [], 0, 1);
para_wFB1 = fmincon(@(x) lossFunc(x, 0.55, 12.72, 0.0), 0.5, [], [], [], [], 0, 1);
para_wFB2 = fmincon(@(x) lossFunc(x, 0.51, 14.80, 0.1), 0.5, [], [], [], [], 0, 1);
...
l3 = plot([0.8, 2, 3.2], [para_woFB; para_wFB1; para_wFB2], '-ok', 'LineWidth', 2);
...
l3 = plot([0.8, 2, 3.2], [12.41, 12.72, 14.80], '-ok', 'LineWidth', 2);
